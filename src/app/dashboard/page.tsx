  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col gap-1">
        <h1 className="text-3xl font-black tracking-tight text-white">Visão Geral</h1>
        <p className="text-slate-400 font-medium">Acompanhe os principais indicadores do seu negócio hoje.</p>
      </div>
        ...
        {stats.map((stat, i) => {
          ...
          return (
            <Link key={i} href={stat.href}>
              <div className={`bg-[#0f172a] border border-slate-800 p-6 rounded-2xl shadow-[0_2px_10px_rgba(0,0,0,0.2)] transition-all duration-300 hover:shadow-lg hover:-translate-y-1 cursor-pointer flex flex-col gap-4 ${stat.borderHover}`}>
                <div className={`p-3 w-fit rounded-xl ${stat.bgLight} ${stat.iconColor}`}>
                  <Icon className="w-6 h-6 stroke-[2.5]" />
                </div>
                <div>
                  <p className="text-sm font-semibold text-slate-400 uppercase tracking-wider">{stat.label}</p>
                  <p className={`text-3xl font-black mt-1 ${stat.valueColor || "text-white"}`}>{stat.value}</p>
                </div>
              </div>
            </Link>
          );
      ...
      <div className="mt-8 bg-gradient-to-br from-blue-900 to-[#0a0f1c] border border-blue-900/50 p-8 rounded-3xl shadow-xl shadow-blue-900/10 text-white flex flex-col md:flex-row items-center gap-6 justify-between relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-blue-200/80 font-medium max-w-xl leading-relaxed">
            Seu ERP está funcionando perfeitamente. Use o menu lateral com ícones coloridos para navegar rapidamente entre os módulos e gerenciar sua empresa.
          </p>
        </div>
        <Link href="/pdv" className="relative z-10">
          <button className="bg-blue-600 text-white hover:bg-blue-500 font-bold py-3 px-6 rounded-xl transition-all shadow-lg shadow-blue-500/20 hover:shadow-xl hover:shadow-blue-500/40 active:scale-95 flex items-center gap-2">
